#!/usr/bin/env bash
# ============================================================================
# fix_pages_idempotent.sh – Idempotent: works in any directory, creates repo if needed.
# ============================================================================
...
REPO_DIR="$PWD"  # start here

echo "=== FIX PAGES (IDEMPOTENT) ==="
echo "Starting directory: $REPO_DIR"

# ----------------------------------------------------------------------
# 1. Determine if we are already in the repo or need to clone.
# ----------------------------------------------------------------------
if [ -d ".git" ] && git remote get-url origin 2>/dev/null | grep -q "$REPO"; then
    ...
    echo "Directory is empty. Cloning repo here..."
    ...
    echo "⚠️  Directory is not empty and not the repo."
    echo "Creating a subdirectory and cloning there..."
    mkdir -p "$REPO" || { echo "Cannot create subdirectory"; exit 1; }
    cd "$REPO" || exit 1
    git clone "https://github.com/$OWNER/$REPO.git" . || {
        echo "❌ Clone failed."
        exit 1
